...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit raycasthit, 100, noSphere))
            {
                ...
                //Raycast sichtbar machen für Ray Debuging
                ...
                //Checks if the ray hits a Turret Base and the Normaltower icon is clicked

                CheckAndBuildNormalTower(raycasthit);

                //Checks if the ray hits a Turret Base and the Freezetower icon is clicked

                CheckAndBuildFreezeTower(raycasthit);

                //Checks if the ray hits a Turret Base and the Toxictower icon is clicked

                CheckAndBuildToxicTower(raycasthit);

                //Checks if the ray hits a Road and the Bomb icon is clicked

                CheckAndBuildBombs(raycasthit);

                //Checks if theres a differnt upgrade menu open => closes it when selecting a new one

                UpgradeMenuSwitch(raycasthit);
    ...
    private void CheckAndBuildNormalTower(RaycastHit raycasthit)
    {
    ...
    private void CheckAndBuildFreezeTower(RaycastHit raycasthit)
    {
        if (raycasthit.transform.CompareTag("Turret Base") || raycasthit.transform.CompareTag("Turret Base 2x"))
        {
    ...
    private void CheckAndBuildToxicTower(RaycastHit raycasthit)
    {
        if (raycasthit.transform.CompareTag("Turret Base") || raycasthit.transform.CompareTag("Turret Base 2x"))
        {
    ...
    private void CheckAndBuildBombs(RaycastHit raycasthit)
    {
    ...
    private void UpgradeMenuSwitch(RaycastHit raycasthit)
    {
